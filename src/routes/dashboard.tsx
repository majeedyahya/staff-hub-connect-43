  ...
  head: () => ({
    meta: [
      { title: "Dashboard — Northbeam Staff Information System" },
      {
        name: "description",
        content:
          "Staff and admin overview: headcount, pending registrations, staff on leave and department counts.",
      },
      { property: "og:title", content: "Dashboard — Northbeam Staff Console" },
      {
        property: "og:description",
        content: "Headcount, pending registrations, leave activity and departments at a glance.",
      },
    ],
  }),
...
function DashboardPage() {
  ...
  const myLeaves = leaves.filter((l) => l.user === user?.id);
  const myPortfolio = portfolios.find((p) => p.user === user?.id);
  ...
  return (
    <AppShell title={`Welcome back, ${user?.fullname.split(" ")[0] ?? ""}`}>
